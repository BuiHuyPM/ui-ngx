...
SET jarfile=%BASE%\lib\inergy.jar
SET installDir=%BASE%\data

PUSHD "%BASE%\conf"

java -cp "%jarfile%" -Dloader.main=vn.inergy.server.InergyInstallApplication^
...
if errorlevel 1 (
   @echo Inergy System installation failed!
   POPD
   exit /b %errorlevel%
)
POPD

"%BASE%"inergy.exe install

@ECHO Inergy System installed successfully!
